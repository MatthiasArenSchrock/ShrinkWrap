...
public class SchubsHTest {
    ...
    public void testHuffmanManyThings() throws IOException {
        String manyThingsText = generateRandomString(1000, 's', 'l');
        ...
        manyThingsText = """
                ...
                """;
        manyThingsTest = dir.resolve("ManyThings.txt");
        Files.write(manyThingsTest, manyThingsText.getBytes());
    ...
    public void testHuffmanLongWord() throws IOException {
        ...
        longWord = generateRandomString(1000);
        Files.write(longWordText, longWord.getBytes());
    ...
    private String generateRandomString(int length, char... flags) {
        Random rand = new Random();
        StringBuilder sb = new StringBuilder();
        String f = new String(flags);
        char ch;

        boolean spaces = f.contains("s");
        boolean lineEndings = f.contains("l");

        for (int i = 0; i < length; i++) {
            ch = (char) rand.nextInt(128);
            while (Character.isWhitespace(ch) && !spaces) {
                ch = (char) rand.nextInt(128);
            }

            if (lineEndings && i % 10 == 0) {
                ch = '\n';
            }
